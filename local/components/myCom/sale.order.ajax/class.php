...
class SaleOrderAjax extends \CBitrixComponent
{
	...
	protected function setOrderProperties(Order $order)
	{
		...
		$arrProp = [];
		if ($order->getPersonTypeId() == 2) { // определяем что это юр лица 
			$arrProp = array( // базовые настройки массива
				8 => "Название компании",
				9 => "Юридический адрес",
				10 => "ИНН",
				11 => "КПП",
			...
			$db_props = CIBlockElement::GetProperty(4, $this->arResult["ORDER_PROP"]["USER_PROFILES"][$this->arUserResult["PROFILE_ID"]]["ID_IFOBLOC_ELEMENT"], array("sort" => "asc"), array());
			$arrElenetsProps = [];
			while ($ob = $db_props->GetNext()) {
				$arrElenetsProps[$ob["CODE"]] = $ob["VALUE"];
			}
			$arrProp[8] = $arrElenetsProps["COMPANI_NAME"];
			$arrProp[9] = $arrElenetsProps["UR_ADRES"];
			$arrProp[10] = $arrElenetsProps["INN"];
			$arrProp[12] = $arrElenetsProps["CONTACT_NAME"];
			$arrProp[13] = $arrElenetsProps["EMAIL"];
			$arrProp[14] = $arrElenetsProps["PHONE"];

			// $arrProp = $this->arUserResult['ORDER_PROP'];
		} else // если пользователь "тип" физ лицо
		{
			$arrProp = $this->arUserResult['ORDER_PROP'];
		}
		$res = $propertyCollection->setValuesFromPost(['PROPERTIES' => $arrProp], []); //в орегинале стаял $this->arUserResult['ORDER_PROP'] заменен на $arrProp
	...
	protected function checkSocServicesAuthForm()
	{
		...
		if (Loader::includeModule("socialservices") && $arResult["ALLOW_SOCSERV_AUTHORIZATION"] == 'Y') {
			...
			if (!empty($arServices)) {
				...
				if (
					isset($this->request["auth_service_id"])
	...
	protected function getOrderPropFormatted($arProperty, &$arDeleteFieldLocation = [])
	{
		...
		if (
			$arProperty['REQUIRED'] === 'Y' || $arProperty['IS_PROFILE_NAME'] === 'Y'
	...
	protected function initUserProfiles(Order $order, $isPersonTypeChanged)
	{
		...
		if ($order->getPersonTypeId() == 2) { // нужен чтобы разделять типы плательщиков на юр и обычные
			if (!$this->request->isPost()) {
				...
				while ($tmp = $res->GetNext()) {
					CSaleOrderUserProps::Delete($tmp["ID"]);
				}


				CModule::IncludeModule('iblock');
				$dbUsers = CIBlockElement::GetList(
					array(),
					array(
						"IBLOCK_ID" => 4,
						"PROPERTY_MENEGER" => $order->getUserId(),
					),
					false,
					false,
					array("ID", "NAME")
				);
				while ($arUserProfiles = $dbUsers->GetNext()) {

					$arFields = array(
						"NAME" => $arUserProfiles["NAME"],
						"USER_ID" => $order->getUserId(),
						"PERSON_TYPE_ID" => 2
					);
					$id = CSaleOrderUserProps::Add($arFields);

					$idInfoblocElements[$id] = $arUserProfiles["ID"];
				}
			}
			...
			if(($dbUserProfiles->SelectedRowsCount()==0)&&$this->request->isPost())
			{
				CModule::IncludeModule('iblock');
				$dbUsers = CIBlockElement::GetList(
					array(),
		...
		while ($arUserProfiles = $dbUserProfiles->GetNext()) {
			...
			if (!empty($idInfoblocElements)) {
				foreach ($idInfoblocElements as $id => $idIfobloc) {
					if ($arUserProfiles["ID"] == $id) {
						$arUserProfiles["ID_IFOBLOC_ELEMENT"] = $idIfobloc;
					}
				}
			}

			if ($this->request->isPost()) {
				foreach ($arrDbUser as $user) {
					if ($user["NAME"] == $arUserProfiles["NAME"]) {
						$arUserProfiles["ID_IFOBLOC_ELEMENT"] = $user["ID"];
					}
				}
			}
	...
	public static function scaleImages(&$result, $scale = '')
	{
		if (!empty($result) && is_array($result)) {
			...
			if (!empty($result['INNER_PAY_SYSTEM']) && is_array($result['INNER_PAY_SYSTEM']) && !empty($result['INNER_PAY_SYSTEM']["LOGOTIP"])) {
				self::resizeImage(
					$result['INNER_PAY_SYSTEM'],
					'LOGOTIP',
					$result['INNER_PAY_SYSTEM']["LOGOTIP"],
					["width" => 600, "height" => 600],
					["width" => 95, "height" => 55],
					$scale
				);
